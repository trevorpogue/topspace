...
(defun vcm--center-offset ()
		...
		(let ((center-offset (/ (* (- (vcm--window-lines) vcm-buffer-lines
																																) 31) 64)))
...
(defun vcm--add-to-scroll-offset (direction)
		...
		(when (= (vcm--top-line) 1)
				...
				(unless (and (> direction 0)
																	(>= (vcm--overlay-size) (- (vcm--window-lines) 5)))
...
(defun vcm--set-prev-buffer-lines (&optional arg0 arg1 arg2)
		...
		(setq-local vcm-prev-buf-size (count-screen-lines (point-min) (point-max))))
...
(defun vcm--update-buffer-lines ()
		(setq-local vcm-buffer-lines (count-screen-lines (point-min) (point-max))))
...
(defvar vcm--hook-alist
		'(
				...
				(before-change-functions . vcm--set-prev-buffer-lines)
				(after-change-functions . vcm--recenter-keep-scroll-after-change)
